...
export function stringify(
  xml: Record<PropertyKey, unknown>,
...
function stringifyXML(
  ...
  if (doctype) {
    ...
    if (elements.length) {
      ...
      document.string += `\n${indent2}[\n${
        elements.map((name) =>
          `${indent2 + " ".repeat(indentSize)}<!ELEMENT ${name} ${doctype
            ?.[name]}>`
        ).join("\n")
      }\n${indent2}]\n`;
    }
  ...
  // Handle text nodes
  if (content) {
    const lines = replace(content as string, { nullToEmpty }).split(
      "\n",
    ) as string[];
    if (lines.length > 1) {
      document.string += `\n${
