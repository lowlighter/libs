...
test("`Resource` cannot be instantiated without extending constructor with `Resource.with()`", () => {
  expect(() => new Resource()).toThrow(ReferenceError)
})

test("`Resource.with()` returns a new resource constructor", async () => {
...
test("`Resource.with() returns a new resource constructor with correct typings and can be extended", () => {
...
test("`Resource.constructor()` fetches back data from store when id is given", async () => {
...
test("`Resource.constructor()` prevents duplicates", async () => {
...
test("`Resource.data` is not writable", async () => {
...
test("`Resource.{id, created, updated}` are readonly", async () => {
...
test("`Resource.patch()` patches resource data", async () => {
...
test("`Resource.load()` loads data from store", async () => {
...
test("`Resource.save()` saves data into store", async () => {
...
test("`Resource.save()` prevents saving invalid data into store", async () => {
...
test("`Resource.save()` initialize defaults value in store", async () => {
...
test("`Resource.delete()` deletes data from store", async () => {
...
test("`Resource.has()` tests resource presence in store", async () => {
...
test("`Resource.get()` gets resource from store and is the same instance each time", async () => {
...
test("`Resource.get()` gets resource data from store", async () => {
...
test("`Resource.delete()` deletes resource from store", async () => {
...
test("`Resource.list()` lists resources from store", async () => {
...
test("`Resource.list()` lists resource data from store", async () => {
...
test("`Resource.schema` returns a JSON schema of the resource", async () => {
