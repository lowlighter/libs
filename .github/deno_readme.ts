...
for await (const { path } of expandGlob(`*/deno.jsonc`, { root })) {
  const { icon, description, supported = [], playground, npm } = JSONC.parse(await Deno.readTextFile(path)) as Record<string, unknown>
  ...
  <tr>
    ...
    <th rowspan="4">${(supported as string[]).map((platform) => `<img height="18px" src="https://jsr.io/logos/${platform}.svg">`).join("")}</th>
