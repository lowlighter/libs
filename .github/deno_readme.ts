...
import { basename, dirname, fromFileUrl, resolve } from "jsr:@std/path@^1.1.2"
...
for await (const { path } of expandGlob(`*/deno.jsonc`, { root })) {
  ...
  <tr>
    ...
    <th rowspan="${4 + (denoland ? 1 : 0)}">${Object.entries(supported as Record<string, boolean>).filter(([_, value]) => value).map(([platform]) => `<img height="18px" src="https://jsr.io/logos/${platform}.svg">`).join("")}</th>
