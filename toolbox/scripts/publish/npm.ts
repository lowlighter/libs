...
export async function publish({ package: pkg = Deno.cwd(), scope = "@", private: restricted = false, dryrun = false }: Options = {}): Promise<{ name: string; version: string; directory: string }> {
  ...
  await command("npm", ["publish", `${directory}/package`, "--access", restricted ? "private" : "public", ...(dryrun ? ["--dry-run"] : [])], { throw: true })
...
export type Options = {
  ...
  scope?: string
  /** Whether to publish with private registry access. */
  private?: boolean
...
// Entry point
if (import.meta.main) {
  const args = parseArgs(Deno.args, { string: ["package", "scope"], boolean: ["private", "dryrun", "help"], negatable: ["private"], default: { scope: "@", private: false } })
  if (args.help) {
    console.error("Usage: deno run --allow-all toolbox/scripts/publish_npm.ts [options]")
    console.error("")
    ...
    console.error("  --no-private         Publish with public registry access instead of private")
