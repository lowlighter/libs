      ...
      new Promise<string>(async (resolve, reject) => {
        try {
          if (!await exists(path)) {
            const url =
              `https://github.com/Sparticuz/chromium/releases/download/v${version}/chromium-v${version}-pack.${
                { x86_64: "x64", aarch64: "arm64" }[arch] ?? "unknown"
              }.tar`;
            for await (
              const entry of await fetch(url).then((response) =>
                response.body!.pipeThrough(new UntarStream())
              )
            ) {
              if (!/^chromium(?:\.br)$/.test(basename(entry.path))) {
