...
async function main(): Promise<void> {
  ...
  if (args.help) {
    ...
    console.error(`  deno run --allow-read --allow-write --allow-run --ignore-env jsr:@libs/database/generate users.sql projects.sql`)
    console.error(`  deno run --allow-read --allow-write --allow-run --ignore-env jsr:@libs/database/generate -o queries.gen.ts users.sql projects.sql`)
    return
  }
  ...
  // Load the native compiler only when generation is requested
  const { generate, generateTables } = await import("./generator.ts")
