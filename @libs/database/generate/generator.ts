  ...
  for (const source of sources) {
    ...
    const flush = async () => {
      ...
      for (const phase of ["pre", "post"] as const) {
        for (const [index, name] of method.hooks[phase].entries()) {
          ...
          entries.push(phase === "pre" ? `_Hook<${args}, ${tuple}[1], C>` : `_PostHook<${tuple}[0], ${tuple}[1], C>`)
    ...
    for (const line of source.split(/\r?\n/)) {
      ...
      if (directive)
        ...
        sql.push(line)
      else if (/^\s*--\s*<\/?(?:sqlite|postgres)>\s*$/.test(line))
        throw new SyntaxError("Backend blocks must follow a query signature")
...
function shape(type: ts.TypeNode): "array" | "one" | "optional" | "nullable" | "void" {
  ...
  if (ts.isUnionTypeNode(type)) {
    if (type.types.some((type) => shape(type) === "array"))
      return "array"
...
async function bind(source: string, resolve: (expression: string) => Promise<string>, constant: (reference: string) => Promise<{ sqlite: string; postgres: string }>): Promise<{ sqlite: string; postgres: string; parameters: { sqlite: string[]; postgres: string[] } }> {
  ...
  while (index < source.length) {
    ...
    if (rest.startsWith("--")) {
      ...
      // Interpret standalone backend markers only outside strings and block comments
      const marker = /^--\s*<(\/?)(sqlite|postgres)>\s*$/.exec(source.slice(index, end))
      const standalone = !source.slice(source.lastIndexOf("\n", index - 1) + 1, index).trim()
      if (marker && standalone) {
        const name = marker[2] as "sqlite" | "postgres"
        if (marker[1]) {
          if (backend !== name)
            throw new SyntaxError("Mismatched backend closing tag: " + name)
          backend = undefined
        } else {
          if (backend)
            throw new SyntaxError("Backend blocks cannot be nested")
          backend = name
        }
        index = end
        continue
      ...
      if (backend !== "postgres") {
        parameters.sqlite.push(expression)
        sqlite += "?"
      }
      if (backend !== "sqlite") {
        parameters.postgres.push(expression)
        postgres += "$" + parameters.postgres.length
      }
      index = end + 1
      continue
    }
    // Keep common SQL and the current backend-specific fragment
    if (backend !== "postgres")
  ...
  if (backend)
    throw new SyntaxError("Unclosed backend block: " + backend)
  if (!sqlite.trim() || !postgres.trim())
    throw new SyntaxError("Query must contain SQL for both backends")
...
function expression(node: ts.TypeNode, models: ReadonlySet<string>, bounds: ReadonlyMap<string, ts.TypeNode> = new Map()): string {
  ...
  if (ts.isTypeLiteralNode(node)) {
    const properties = node.members.map((member) => {
      ...
      return `${JSON.stringify(name)}: ${expression(member.type, models, bounds)}${member.postfixToken?.kind === ts.SyntaxKind.QuestionToken ? ".optional()" : ""}`
    })
...
class Compiler implements AsyncDisposable {
  ...
  async parse(text: string, check = true): Promise<ts.SourceFile> {
    ...
    this.#path = resolve(`.database-generator/query${this.#index++}.ts`)
    this.#text = text
    const snapshot = await this.#api.updateSnapshot({ openFiles: [this.#path], closeFiles: [previous], fileChanges: { created: [this.#path], deleted: [previous] } })
  ...
  async [Symbol.asyncDispose](): Promise<void> {
    // TypeScript 7.0.2 closes stdin but does not await its child's close event
    const child = Reflect.get(Reflect.get(this.#api, "client"), "process") as ChildProcess | undefined
    const closed = child && child.exitCode === null && child.signalCode === null ? new Promise<void>((resolve) => child.once("close", () => resolve())) : Promise.resolve()
