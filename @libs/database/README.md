...
More complex type expressions can be used (using unions, intersections, `Pick`, `Omit`, `Partial`, `NonNullable`, etc.) but very complex expressions may not be fully supported and could lead to generation errors. If you encounter such issues, consider implementing your own
validation hooks.
...
Its first argument is always `{ context }`, where `context` is a fresh empty record when omitted. Query inputs follow, then any declared hook arguments. If it returns a defined value, it replaces the original query parameters with the returned tuple (note that it is still
validated against the original types).
...
Its first argument is always `{ context, result }`, where `context` is a fresh empty record when no query context was supplied. Declared hook arguments follow this object, regardless of whether context was provided. If it returns a defined value, that value replaces the original
result.
