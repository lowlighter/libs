...
Most packages in this repository honors [web standards](https://developer.mozilla.org/en-US/docs/Learn/Getting_started_with_the_web/The_web_and_web_standards) which means they can be used with many runtimes such as [deno](https://deno.com) , [Node.js](https://nodejs.org),
[bun](https://bun.sh) and even browsers.

These are published to [jsr.io](https://jsr.io), [npm](https://www.npmjs.com) and [deno.land](https://deno.land/x) registries.

See table below for a list of available libraries and their features, compatibility, test coverage and playgrounds.

Releases are fully automated and are published on every push to the `main` branch and respect [semver](https://semver.org) versioning. Each package from this repository is versioned independently.
