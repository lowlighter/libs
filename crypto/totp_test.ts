...
test("`otpsecret()` returns a string", () => {
  expect(typeof otpsecret()).toBe("string")
})

test("`otpauth()` throws when either issuer or account contains a colon character", () => {
...
test("`verify()` returns false if token is invalid", async () => {
