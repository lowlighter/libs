...
test("`unmap()` handles destructured imports", () => {
...
test("`unmap()` handles multiline imports", () => {
  expect(unmap(
    `import {
    foo,
    bar as qux,
...
test("`unmap()` handles directory imports", () => {
...
test("`unmap()` does not remap unmapped paths", () => {
